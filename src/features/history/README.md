# History Feature (캠페인 히스토리 기능)

## 📌 이 폴더는 무엇인가요?

사용자의 **캠페인 참여 히스토리**를 조회하고 관리하는 기능을 담당하는 폴더입니다.
신청 중, 진행 중, 완료된 캠페인을 탭으로 구분하여 보여줍니다.
...
| 상태        | 설명                         | 사용자가 할 일         |
| ----------- | ---------------------------- | ---------------------- |
| `applying`  | 캠페인 신청 중               | 선정 대기              |
| `selected`  | 선정됨                       | 제품 수령 후 리뷰 작성 |
| `reviewing` | 리뷰 작성 중                 | 리뷰 완성 및 제출      |
| `completed` | 캠페인 완료 (리뷰 제출 완료) | 추가 작업 없음         |
...
function HistoryPage() {
  ...
  if (!data || data.length === 0) return <div>캠페인이 없습니다</div>;
